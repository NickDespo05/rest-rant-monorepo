...
router.get("/:placeId", async (req, res) => {
    ...
    if (isNaN(placeId)) {
        ...
        if (!place) {
            res.status(404).json({
                message: `Could not find place with id "${placeId}"`,
            });
        } else {
            res.json(place);
        }
...
router.post("/:placeId/comments", async (req, res) => {
    ...
    //     if (method === "Bearer") {
    //         const result = await jwt.decode(process.env.JWT_SECRET);
...
router.delete("/:placeId/comments/:commentId", async (req, res) => {
    ...
    if (isNaN(placeId)) {
        ...
        if (!comment) {
            ...
            res.status(403).json({
                message: "you dont have permission to delete the comment",
            });
        } else {
